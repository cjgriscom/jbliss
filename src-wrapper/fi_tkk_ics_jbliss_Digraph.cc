...
#include <stdint.h>
#include "graph.hh"
#include "fi_tkk_ics_jbliss_Digraph.h"

static bliss::Digraph *_j2c(const jlong j_ptr)
{
  return reinterpret_cast<bliss::Digraph *>(static_cast<intptr_t>(j_ptr));
}

static jlong _c2j(bliss::Digraph * const c_ptr)
{
...
{
  bliss::Digraph *graph = new bliss::Digraph();
  assert(graph);
  return _c2j(graph);
}
...
{
  assert(true_bliss != 0);
  bliss::Digraph *graph = _j2c(true_bliss);
  assert(graph);
  delete graph;
}


JNIEXPORT jlong JNICALL Java_fi_tkk_ics_jbliss_Digraph__1read_1dimacs
(JNIEnv *env, jclass c, jstring filename)
{
  ...
  bliss::Digraph *graph = bliss::Digraph::read_dimacs(fp, 0);
  assert(graph);
  fclose(fp); fp = 0;
  return _c2j(graph);
}
...
JNIEXPORT void JNICALL Java_fi_tkk_ics_jbliss_Digraph__1write_1dimacs
...
JNIEXPORT jlong JNICALL Java_fi_tkk_ics_jbliss_Digraph__1permute
  (JNIEnv *env, jobject, jlong true_bliss, jintArray j_perm)
{
  ...
  /* Permute the graph */
  bliss::Digraph *new_graph = graph->permute(perm);
  assert(new_graph);
...
(JNIEnv *env, jobject caller, jlong true_bliss, jobject reporter)
{
  ...
  jclass reporter_class = env->FindClass("fi/tkk/ics/jbliss/Digraph");
